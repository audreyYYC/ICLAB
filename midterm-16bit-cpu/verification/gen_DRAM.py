...
BRANCH_LOOP = 1 # 1: allow branch to backward, 0: only forward branch
PAT_NUM = 4000 # max number of patterns to run (if pc still < 0x3000 after this number, stop)
# =================================
# IDX_MAX = (0x2000 - 0x1000) // 2 # 2048
IDX_MAX = (0x3000 - 0x1000) // 2 # 4096
...
def genRandomInst():
    ...
    jump_from_idx = [150, 181, 187, 699, 707, 725, 750, 850, 1000, 1015, 1032, 1500, 1517, 1531, 1550, 1750]
    jump_to_idx   = [182, 188, 156, 731, 751, 700, 725, 851, 1020, 1034, 1001, 1532, 1551, 1501, 1520, 1767]
    addr = 0x1000
    insts = []
    for idx in range(16):
        rt = idx
